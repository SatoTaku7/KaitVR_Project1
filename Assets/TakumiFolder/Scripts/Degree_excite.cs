...
{
    private float clear_criteria;//この数値と音量が近いほど盛り上がり度が高くなる　ランダムな時間ごとに★ランダムな数値になる
    ...
    void After_Tutorial()
    {
        ...
        if ((count >= criteria_span)&&(count!=0))
        {
            ...
            clear_criteria = 50;//ランダムで0～100の基準となる数値を決める
        ...
        //0.288   -0.896
        transform.rotation = Quaternion.Euler(0, 0, Mathf.Abs(clear_criteria - volume_slider.value)*1.2f);
        Debug.Log(Mathf.Abs(clear_criteria - volume_slider.value));
    }
